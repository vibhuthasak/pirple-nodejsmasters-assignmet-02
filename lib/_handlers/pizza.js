          ...
          if(!err && userData){
            ...
            _data.read('items', 'pizza', function(err, pizzaList){
              if(!err && pizzaList){
                ...
                if(helpers.IdIsValid(pizzaList, pizzaId)){
                  // Checking pizza if already on the user's cart
                  if(helpers.IdIsValid(userData.cart, pizzaId)){
                    // pizza id is already on the list
                    // Can't POST
                    callback(400, {
                      'Error': 'Pizza id is already on the user\'s cart'
                    });
                  } else {
                    // Add new pizza id and count to the user's cart
                    var pizzaCartObject = {
                      'id': pizzaId,
                      'count': pizzaCount,
                      'price': pizzaCount*helpers.IdIsValid(pizzaList, pizzaId)['price']
                    }
